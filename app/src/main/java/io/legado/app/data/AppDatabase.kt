...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private const val DATABASE_NAME = "mlbook.db"
        ...
        private val dbCallback = object : Callback() {
            ...
            override fun onOpen(db: SupportSQLiteDatabase) {
                db.execSQL(
                    ...
                    select ${AppConst.bookGroupAllId}, '${appCtx.getString(R.string.db_bookshelf_all)}', -10, 1
                    where not exists (select * from book_groups where groupId = ${AppConst.bookGroupAllId})"""
                )
                db.execSQL(
                    """insert into book_groups(groupId, groupName, 'order', show) 
                    select ${AppConst.bookGroupLocalId}, '${appCtx.getString(R.string.db_bookshelf_local)}', -9, 1
                    where not exists (select * from book_groups where groupId = ${AppConst.bookGroupLocalId})"""
                )
                db.execSQL(
                    """insert into book_groups(groupId, groupName, 'order', show) 
                    select ${AppConst.bookGroupAudioId}, '${appCtx.getString(R.string.db_bookshelf_audio)}', -8, 1
                    where not exists (select * from book_groups where groupId = ${AppConst.bookGroupAudioId})"""
                )
                db.execSQL(
                    """insert into book_groups(groupId, groupName, 'order', show) 
                    select ${AppConst.bookGroupNoneId}, '${appCtx.getString(R.string.db_bookshelf_unknow)}', -7, 1
